...
\qsong{First we should talk the motivation of this study...} In Sec~\ref{sec:op_over_infinite_plane}, we study the network-level packet loss rate among with other metrics. However, the network-level performance metrics can only reflect the average quality of service proposed to devices. For example, it is very possible that packet loss rate of the devices at the border of BS coverage is higher than the expected value. It is necessary to use a more refined performance metric
